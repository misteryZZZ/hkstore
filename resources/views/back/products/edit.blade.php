...
<div id="product">
	<form class="ui large main form" method="post" autocomplete="off" enctype="multipart/form-data" action="{{ route('products.update', $product->id) }}">
		<div class="field">
			...
			<a class="ui icon labeled circular large yellow button" :class="{disabled: anyInputOff()}" href="{{ route('products') }}">
		...
		<div class="ui one column grid">
			<div class="column tabs">
			  ...
			  <div class="ui tab segment active" data-tab="overview">
			  	...
			  	<div class="field">
			  		...
			  		<div class="ui selection floating search dropdown">
						  <input type="hidden" name="type" @change="setItemType($event)" value="{{ old('type', $product->type ?? config('app.default_product_type', '-')) }}">
									...
									<div class="w-100" v-else>
										...
										<div class="ui floating circular fluid dropdown large blue basic button mx-0 files" :class="{disabled: inputIsOff('download') || fileId}">
											...
											<div class="menu">
												...
												<div class="item" @click="browserMainFile('one_drive')">{{ __('One Drive') }}</div>
												@endif --}}
												...
												<div class="item" @click="browserMainFile('main_file_upload_link')">{{ __('Upload link') }}</div>
												
												@if(!isFolderProcess())
												<div class="item" @click="browserMainFile('main_file_download_link')">{{ __('Download link') }}</div>
												@endif
											</div>
										</div>
										...
										@if(!isFolderProcess())
										<input type="url" name="main_file_download_link" :class="{disabled: inputIsOff('download')}" class="mt-1 {{ old('main_file_download_link', $product->direct_download_link) ? 'active' : '' }}" placeholder="{{ __('Download link') }}" value="{{ old('main_file_download_link', $product->direct_download_link) }}"  @change="setDefaultDrive">
										@endif
									</div>
								</div>
							</div>
							...
							<div class="fluid card">
								...
								<div class="content">
									<div v-if="oldUploads.cover.length">
									    ...
									    @if($cover)
											<img src="{{ asset_("storage/covers/{$cover}") }}">
											@endif
									  </div>
									</div>
									<div v-else>
							...
							<div class="fluid card">
								...
								<div class="content">
									...
									<div class="w-100" v-else>
										...
										<input type="url" name="preview_upload_link" :class="{disabled: inputIsOff('preview')}" placeholder="{{ __('Upload link') }}" value="{{ old('preview_upload_link') }}" class="mt-1">

										<input type="url" name="preview_direct_link" :class="{disabled: inputIsOff('preview')}" placeholder="{{ __('Direct link') }}" value="{{ old('preview_direct_link', $product->preview_direct_link) }}" class="mt-1 {{ old('preview_direct_link', $product->preview_direct_link) ? 'active' : '' }}">

										<div class="ui floating circular fluid dropdown large blue basic button mt-1">
											<input type="hidden" name="preview_type" value="{{ old('preview_type', $product->preview_type ?? null) }}">
											<div class="text">...</div>
											<div class="menu">
			  ...
			  <div class="ui tab segment" data-tab="description">
					<textarea name="overview" class="summernote">{{ old('overview', $product->overview) }}</textarea>
			  </div>

			  <div class="ui tab segment" data-tab="hidden-content">
					<textarea name="hidden_content" class="summernote">{{ old('hidden_content', $product->hidden_content ?? null) }}</textarea>
			  </div>
			  	...
			  	<div class="table wrapper free">
				  	<table class="ui basic table unstackable w-100">
					  	<tr>
					  		...
					  		<td><input type="text" name="free[from]" value="{{ old('free.from', $product->free->from ?? null) }}" placeholder="{{ __('From') }} : YYYY-MM-DD"></td>
					  		<td><input type="text" name="free[to]" value="{{ old('free.to', $product->free->to ?? null) }}" placeholder="{{ __('To') }} : YYYY-MM-DD"></td>
					  	</tr>
					  </table>
				  </div>
				  ...
				  <div class="table wrapper promo_price">
				  	<table class="ui basic table unstackable w-100">
					  	<tr>
					  		...
					  		<td><input type="text" name="promotional_price_time[from]" value="{{ old('promotional_price_time.from', $product->promotional_price_time->from ?? null) }}" placeholder="{{ __('From') }} : YYYY-MM-DD"></td>
					  		<td><input type="text" name="promotional_price_time[to]" value="{{ old('promotional_price_time.to', $product->promotional_price_time->to ?? null) }}" placeholder="{{ __('To') }} : YYYY-MM-DD"></td>
					  	</tr>
					  </table>
				  </div>
			  </div>
...
<script type="application/javascript">
	...
	var app = new Vue({
  	...
  	data: {
  		...
  		localFileName: '',
  		itemType: '{{ old('type', $product->type ?? config('app.default_product_type') ?? '-') }}',
	...
	function savePeaks(previewUrl, filename = null)
	{
		var wSuffer = WaveSurfer.create({
			...
			wSuffer.once('ready', () => 
			{
					...
					{
						$.post("{{ route('products.save_wave') }}", { filename: filename, peaks: res, id: '{{ $product->id }}' })
						.always(function()
						{
	...
	function savePeaksFromTempUrl(url)
	{
		...
		$.post('{{ route('products.get_temp_url') }}', {url: url, id: '{{ $product->id }}'})
