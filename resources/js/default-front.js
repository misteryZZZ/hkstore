...
window.app = new Vue(
{
		...
		checkout: function(e)
		{
			if(this.totalAmount > 0)
			{
				if(this.paymentProcessor)
				{
					...
					else if(this.paymentProcessor === 'paytr' && this.paymentProcessors.paytr)
					{
						var payload = {
							processor: "paytr",
							coupon: this.couponRes.status ? this.couponRes.coupon.code : null,
						...
						.done(function(data)
						{
							...
							$('#paytr-modal .content').html(data.payment_form);
							$('#paytr-modal').modal({
								closable: false
							}).modal('show');
						})
    ...
    {
    	var string = this.translation[key] || null;
    	...
    	if(this.guestAccessToken.length)
    	{
    		...
    		.done(function(data)
    		{
    			if(data.hasOwnProperty('products'))
    			{
    				...
    				{
	    				app.userMessage = app.__('No items found for the given token.');
    		...
    		.fail(function(data)
    		{
    			app.userMessage = data.responseJSON.message;
    ...
    setPrice: function(e)
    {
    	...
    	Vue.nextTick(()=>
    	{
    		var hasPromo = this.itemPrices[this.licenseId]['promo_price'] !== null;
    ...
    removeRecentViewedItem: function(key)
	  {
	  	if(Object.keys(this.recentlyViewedItems).indexOf(key.toString()) >= 0)
	  	{
	  		...
	  		Vue.nextTick(function()
	  		{
	  			localStorage.setItem('recentlyViewedItems', JSON.stringify(app.recentlyViewedItems));
	  		})
	  	}
	  }
	},
...
$(()=>
{
	...
	$(document).on('click', '#top-menu .dropdown.notifications .item:not(.all), #notifications .unstackable.items a.item, \
		#user .notifications .items .item.unread, #guest .notifications .items .item.unread', 
  function()
  {
  ...
  $(document).on('click', '#reactions .header a', function()
  {
	...
	{
		$('#item .r-side').transition('fly left');
	})


	$('#item .l-side .top.menu a.item:not(#item-r-side-toggler)')
	.on('click', function()
	{
